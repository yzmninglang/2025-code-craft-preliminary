...
int tag_block_address[MAX_DISK_NUM][MAX_TAG_NUM];
...
void do_object_write(int* object_unit, int* disk_unit, int size, int object_id)
{
    ...
    int start_address = tag_block_address[object[object_id].main_disk_num][object[object_id].tag];  // 每个TAG分区在各个盘中的首地址（相同）
...
void cut_tag() {
    for (int tag_id = 1; tag_id <= M; tag_id++) {
        int main_disk = (tag_id - 1) % N + 1;
        int tag_id_ptr = (tag_id - 1) * V / M + 1;
        tag_block_address[main_disk][tag_id] = tag_id_ptr;  // 两个副本盘可以由main_disk获取
    }
}
...
void write_action()  // 对象写入事件
{
    ...
    for (int i = 1; i <= n_write; i++) {
        ...
        for (int j = 1; j <= REP_NUM; j++) {
            ...
            object[id].replica[j] = (object[id].main_disk_num + j - 2) % N + 1;  // 得到主盘与两个副本盘编号
