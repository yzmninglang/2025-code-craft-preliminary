...
#define ROLL_TAG_TS_FRE (15)  // 14250094.76 (45->24)
#define ABORT_LOW_SCORE_REQ_TS_FRE (100)  // 14250910.7 (105->100)
...
void read_action()  // 对象读取事件
{
    ...
    static std::vector<std::unordered_set<int>> mycount(MAX_DISK_NUM);
    std::mt19937 g(TS);
    ...
    for (int i = 1; i <= N; i++) {  // 对每个磁头都进行操作
        ...
        while (token > 0) {
            ...
            if ((TS - 1) % 1800 == 0) {
                ...
                // ++mycount[i];
                if(mycount[i].count(most_fre_index[(i+mycount_-1)%N+1])==0){
                    disk_head[i].pos = tag_block_address[most_fre_index[(i+mycount_-1)%N+1]];
                    mycount[i].insert(most_fre_index[(i+mycount_-1)%N+1]);
                }
                else{
                    disk_head[i].pos = tag_block_address[most_fre_index[(i+mycount_-1)%N+1]]+length;
                    mycount[i].erase(most_fre_index[(i+mycount_-1)%N+1]);
                }
